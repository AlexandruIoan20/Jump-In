...
// Functii de test
void afisare_valori_matrice() {
    // Afișarea axei X (coloane)
    cout << "    "; // Spațiu pentru alinierea axei Y
    for (int j = 0; j < N; j++) {
        cout << setw(3) << j; // Afișează indexul coloanei
    }
    cout << '\n';

    // Linie de separare pentru a evidenția matricea
    cout << "   ";
    for (int j = 0; j < N; j++) {
        cout << "---";
    }
    ...
    // Afișarea matricei cu axa Y (rânduri)
    for (int i = 0; i < N; i++) {
        cout << setw(2) << i << " |"; // Afișează indexul rândului
        for (int j = 0; j < N; j++) {
            cout << setw(3) << matrice_joc[i][j].val; // Afișează valoarea
        }
        cout << '\n';
    }
}
